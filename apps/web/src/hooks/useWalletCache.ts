...
export function useWalletConnection() {
  ...
  return useQuery({
    queryKey: [...walletQueryKeys.connection(), wallet?.adapter?.name, connected, connecting, disconnecting],
    queryFn: () => ({
      wallet,
      connected,
      connecting,
...
export function useInvalidateWalletCache() {
  ...
  return {
    invalidateAll: () => queryClient.invalidateQueries({ queryKey: walletQueryKeys.all }),
    invalidateAddress: () => queryClient.invalidateQueries({ queryKey: walletQueryKeys.address() }),
    invalidatePublicKey: () => queryClient.invalidateQueries({ queryKey: walletQueryKeys.publicKey() }),
    invalidateConnection: () => queryClient.invalidateQueries({ queryKey: walletQueryKeys.connection() }),
    invalidateAdapter: () => queryClient.invalidateQueries({ queryKey: walletQueryKeys.adapter() }),
  };
}
