...
describe("errorHandling", () => {
  ...
  describe("createTransactionError", () => {
    ...
    it("should handle unknown error types", () => {
      const result = createTransactionError({
        code: 500,
        data: "Server error",
      });
  ...
  describe("validateRequired", () => {
    it("should pass for valid values", () => {
      expect(() =>
        validateRequired("valid string", "test field"),
      ).not.toThrow();
    ...
    it("should throw for null", () => {
      expect(() => validateRequired(null, "null field")).toThrow(
        LiquidityError,
      );
      expect(() => validateRequired(null, "null field")).toThrow(
    ...
    it("should throw for undefined", () => {
      expect(() => validateRequired(undefined, "undefined field")).toThrow(
        LiquidityError,
      );
      expect(() => validateRequired(undefined, "undefined field")).toThrow(
  ...
  describe("validateWalletConnection", () => {
    it("should pass for valid wallet connection", () => {
      ...
      expect(() =>
        validateWalletConnection(publicKey, walletAdapter),
      ).not.toThrow();
    });
    it("should throw for missing public key", () => {
      const walletAdapter = { wallet: { name: "Phantom" } };
      expect(() => validateWalletConnection(null, walletAdapter)).toThrow(
        "Wallet public key is required",
      );
    });
    it("should throw for missing wallet adapter", () => {
      const publicKey = { toBase58: () => "valid-key" };
      expect(() => validateWalletConnection(publicKey, null)).toThrow(
        "Wallet adapter is required",
      );
    });
    it("should throw for wallet adapter without wallet", () => {
      const publicKey = { toBase58: () => "valid-key" };
      const walletAdapter = { wallet: null };
      expect(() => validateWalletConnection(publicKey, walletAdapter)).toThrow(
        "Wallet adapter is required",
      );
    });
