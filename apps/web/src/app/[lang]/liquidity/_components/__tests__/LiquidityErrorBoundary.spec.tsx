...
vi.mock('@dex-web/ui', () => ({
  ...
  Icon: ({ name, className }: { name: string; className?: string }) => <span className={className}>{name}</span>,
  Text: {
    Body1: ({ children, className }: { children: React.ReactNode; className?: string }) => <p className={className}>{children}</p>,
    Body2: ({ children, className }: { children: React.ReactNode; className?: string }) => <p className={className}>{children}</p>,
  },
}));
...
describe('LiquidityErrorBoundary', () => {
  ...
  describe('Error Severity Detection', () => {
    it('should detect high severity for security errors', () => {
      ...
      const container = screen.getByText('wallet security error').closest('div');
  ...
  describe('Specialized Error Boundaries', () => {
    ...
    it('should render LiquidityTransactionErrorBoundary with no retry for transactions', () => {
      ...
      expect(screen.getByText('Transaction Processing Error')).toBeInTheDocument();
      expect(screen.queryByText('Try Again')).not.toBeInTheDocument();
    });
  ...
  describe('Security Warnings', () => {
    it('should show security warning for high severity errors', () => {
      render(
        <LiquidityErrorBoundary>
          <ThrowError shouldThrow={true} errorMessage="transaction security breach" />
        </LiquidityErrorBoundary>
      );

      expect(screen.getByText(/This error may affect your transaction security/)).toBeInTheDocument();
    });

    it('should not show security warning for low severity errors', () => {
      render(
        <LiquidityErrorBoundary>
          <ThrowError shouldThrow={true} errorMessage="display formatting issue" />
        </LiquidityErrorBoundary>
      );

      expect(screen.queryByText(/This error may affect your transaction security/)).not.toBeInTheDocument();
    });
  });
});